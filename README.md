...
現在提供了全新的網頁版界面，是使用本工具最簡便的方式，無需記憶命令，所有功能都透過直觀的界面操作：
...
啟動後，在瀏覽器中訪問 http://127.0.0.1:5000 即可使用。詳細說明見 [Web版使用指南](#web版使用指南) 部分。
...
```bash
pip install jieba pandas matplotlib seaborn wordcloud xlsxwriter opencc-python-reimplemented chardet flask flask-cors
```
...
Web版是Chinese Text Analyzer的最新界面，提供直觀的交互體驗和豐富的視覺化選項：
...
Web版提供了一個直觀、功能完整的用戶界面，分為四大部分：

1. **文本分析面板**：
   - 在左側文本框輸入要分析的中文文本
   - 點擊「分析文本」按鈕開始分析
   - 使用「載入範例」可快速載入示例文本
   - 「生成完整報告」開關可選擇是否生成詳細報告
...
3. **結果展示區**：
   分為四個標籤頁，提供不同類型的分析結果：
   - **基本分析**：顯示詞雲、詞頻分布、詞性分布等基本視覺化
   - **進階分析**：提供詞頻餅圖、垂直條形圖、關鍵詞權重等進階視覺化
   - **數據表格**：以表格形式展示詳細分析數據
   - **完整報告**：生成並提供完整HTML報告下載

4. **下載選項**：
   - 每個視覺化圖表右上角有下載按鈕
   - 右下角「下載選項」菜單提供批量下載功能
   - 報告頁面提供完整報告下載和所有圖表打包下載

### Web版特色功能

Web版提供了許多命令行版本沒有的增強功能：

1. **分類展示的視覺化**：
   - 基本視覺化：詞雲、詞頻分布、詞性分布、情感分析、命名實體
   - 進階視覺化：詞頻垂直分布、詞頻餅圖、關鍵詞權重、常見詞組、詞長排序
...
3. **即時預覽功能**：
   - 點擊任何視覺化圖表可在彈出視窗中查看大圖
   - 提供下載當前查看圖表的選項

4. **統一報告生成**：
   生成包含所有分析結果的HTML報告，方便分享和保存
...
2. **批量下載選項**：
   - **下載基本圖表**：僅下載基本分析中的視覺化
   - **下載進階圖表**：僅下載進階分析中的視覺化
   - **下載數據(JSON)**：下載原始分析數據的JSON格式
   - **下載數據(CSV)**：下載詞頻數據的CSV格式
   - **下載所有內容**：下載所有視覺化和數據

3. **報告功能**：
   - **在線查看報告**：瀏覽器中查看完整HTML報告
   - **下載報告**：下載HTML格式的完整分析報告
   - **下載所有圖表**：將所有視覺化圖表打包下載

### 使用建議

1. **分析長文本**：
   - 對於較長文本(>5000字)，分析可能需要幾秒到幾十秒不等
   - 分析過程中會顯示進度指示器

2. **優化查看體驗**：
   - 使用寬屏或大屏幕顯示器以獲得最佳視覺效果
   - 可點擊視覺化圖表查看大圖，便於詳細分析
